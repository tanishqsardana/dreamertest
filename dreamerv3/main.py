...
def main(argv=None):
  ...
  def init():
    ...
    # Initialize JAX backend with fallback mechanisms
    try:
      import os
      import jax
      ...
      # Set environment variables to configure JAX
      ...
      for backend in backends_to_try:
          try:
              ...
              # Make sure backend is initialized by requesting devices
              devices = jax.devices()
              current_backend = xla_bridge.get_backend().platform
              
              print(f"✓ JAX successfully initialized with {current_backend} backend")
              print(f"Available devices: {devices}")
              
              # Successfully initialized
              break
          except Exception as e:
              print(f"Failed to initialize JAX with {backend} backend: {e}")
              if backend == backends_to_try[-1]:
                  # This was our last attempt
                  print("CRITICAL: All backend initialization attempts failed.")
                  print("This will likely cause runtime errors.")
              else:
                  # Try next backend
                  print(f"Trying fallback to {backends_to_try[backends_to_try.index(backend)+1]} backend...")
    except Exception as e:
      print(f"WARNING: JAX module initialization error: {e}")
      print("This will likely cause runtime errors.")
  portal.setup(
